...
// ballistically and die when they fall back to the terrain. Gravity and
// point size scale with the rig so powder reads the same at the giant's
// eye as it did at human scale.

const POOL = 384;
const GRAVITY = -3.2; // gentler than earth — powder floats (physical units)
...
export default class Spray {
  ...
  emit(origin, baseVel, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const j = this.count++;
      this.positions[j * 3 + 0] = origin.x + (Math.random() - 0.5) * 0.03 * s;
      this.positions[j * 3 + 1] = origin.y + Math.random() * 0.02 * s;
      this.positions[j * 3 + 2] = origin.z + (Math.random() - 0.5) * 0.03 * s;
      this.velocities[j * 3 + 0] = baseVel.x * (0.6 + Math.random() * 0.8) + (Math.random() - 0.5) * 0.3 * s;
      this.velocities[j * 3 + 1] = Math.abs(baseVel.y) + (0.4 + Math.random() * 0.7) * s;
      this.velocities[j * 3 + 2] = baseVel.z * (0.6 + Math.random() * 0.8) + (Math.random() - 0.5) * 0.3 * s;
    ...
    while (i < this.count) {
      ...
      const dead =
        ...
        (this.positions[i * 3 + 1] <
          terrainSurfaceY(this.positions[i * 3], this.positions[i * 3 + 2]) - 0.02 * s &&
